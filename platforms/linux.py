...
class LinuxBridge(PlatformBridge):
    ...
    def capabilities(self):
        """xdotool covers the mouse and media keys; notify-send the rest."""
        available = []
        if self._has_xdotool:
            available.extend(["mouse", "media_keys"])
    ...
    def click_mouse(self, button="left"):
        ...
        try:
            subprocess.run(
                ["xdotool", "click", code], timeout=1, capture_output=True,
            )
            return True
        except (subprocess.TimeoutExpired, OSError):
            return False
    ...
    def scroll_mouse(self, amount):
        if not self._has_xdotool or amount == 0:
            return False
        # Buttons 4 and 5 are scroll up and down in the X11 model.
        code = "4" if amount > 0 else "5"
        try:
            for _ in range(min(abs(int(amount)), 10)):
                subprocess.run(["xdotool", "click", code], timeout=1, capture_output=True)
            return True
        except (subprocess.TimeoutExpired, OSError):
            return False
    ...
    def send_media_key(self, key):
        ...
        if not keysym:
            return False
        try:
            subprocess.run(["xdotool", "key", keysym], timeout=2, capture_output=True)
            return True
        except (subprocess.TimeoutExpired, OSError):
            return False
