...
class PlatformBridge(abc.ABC):
    ...
    #
    # These have working defaults that simply report "unsupported", so a
    # bridge only implements what its platform can actually do and callers
    # never have to check which platform they are on.

    def supports(self, capability):
        """
        Whether an optional capability is available here.

        Capabilities: ``mouse``, ``media_keys``, ``notifications``.
        """
        return capability in self.capabilities()

    def capabilities(self):
        """Names of the optional capabilities this bridge implements."""
        return ()

    def move_mouse(self, x, y):
        """Move the system cursor to absolute screen coordinates."""
        return False
    ...
    def send_media_key(self, key):
        """
        Send a media key: ``play_pause``, ``next_track``, ``prev_track``,
